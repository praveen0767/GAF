...
const pages = ['contact', 'categories', 'team', 'solutions', 'how-it-works'];
const baseDir = path.join(__dirname, 'app', '[locale]', '(public)');

pages.forEach(page => {
  const compName = page.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('') + 'Page';
  const capPage = page.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('');
  
  const content = `import { useTranslations } from "next-intl";

export default function ${compName}() {
  const t = useTranslations('${capPage}');
