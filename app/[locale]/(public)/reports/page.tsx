        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-[11px] font-bold text-slate-900 uppercase tracking-widest">{t('chartTitle')}</h2>
              <select className="text-xs font-semibold border border-slate-200 text-slate-600 rounded px-3 py-1.5 bg-slate-50 outline-none">
                <option>{t('timeRange.30days')}</option>
                <option>{t('timeRange.quarter')}</option>
                <option>{t('timeRange.ytd')}</option>
              </select>
            </div>
          ...
          <div className="flex flex-col gap-4">
            {[
              { title: t('metrics.rate'), value: "78%", icon: <TrendingUp className="w-5 h-5 text-emerald-600" />, bg: "bg-emerald-50" },
              { title: t('metrics.categories'), value: "14", icon: <Activity className="w-5 h-5 text-blue-600" />, bg: "bg-blue-50" },
              { title: t('metrics.total'), value: "1,240", icon: <FileText className="w-5 h-5 text-slate-600" />, bg: "bg-slate-100" }
            ].map((metric, i) => (
          ...
          <div className="lg:col-span-3 mt-8">
            <h2 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-4">{t('briefings.title')}</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
              {[
                { month: t('briefings.oct'), desc: t('briefings.octDesc') },
                { month: t('briefings.sep'), desc: t('briefings.sepDesc') },
                { month: t('briefings.aug'), desc: t('briefings.augDesc') },
                { month: t('briefings.jul'), desc: t('briefings.julDesc') }
              ].map((report, i) => (
                <div key={i} className="bg-white border border-slate-200 p-6 rounded-xl hover:border-blue-300 hover:shadow-md transition-all group">
                  <h3 className="text-sm font-bold text-slate-900 mb-1">{report.month}</h3>
                  ...
                  <Link href="#" className="inline-flex items-center text-[11px] font-bold text-blue-600 uppercase tracking-widest group-hover:text-blue-800 transition-colors">
                    {t('briefings.download')} <ArrowRight className="ml-1 w-3 h-3" />
                  </Link>
                </div>
              ))}
            </div>
